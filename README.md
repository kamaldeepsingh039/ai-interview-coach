...
- **Backend:** Python + Flask, one file (`app.py`). Three JSON API routes:
  ...
  - `POST /api/feedback` — sends the question + your answer to the Gemini API, returns feedback
...
- The model is set in app.py as model="gemini-3.5-flash". Google occasionally retires older model names - if you ever
  Get a 404 mentioning a model, check ai.google.dev/gemini-api/docs/models for the current name and swap it in.
